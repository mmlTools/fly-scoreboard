...
// We only need plain HTTP here; make sure cpp-httplib doesn't try to pull OpenSSL.
...
int fly_server_start(const QString &base_dir_q, int preferred_port)
{
    ...
    if (!port) {
        LOGW("HTTP server failed to bind any port in [%d..%d] on 127.0.0.1/0.0.0.0",
    ...
    g_thread = std::thread([host_bound]() {
        LOGI("Web server START on http://%s:%d (doc_root=%s)",
